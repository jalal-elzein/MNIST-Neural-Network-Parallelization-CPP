...
void perceptron() {
    // initialize values for the input of the second layer to 0
    for (int i = 1; i <= n2; ++i) {
		in2[i] = 0.0;
	}

    // initialize values for the input of the third layer to 0
    for (int i = 1; i <= n3; ++i) {
		in3[i] = 0.0;
	}

    // factor in the weights of the first layer to its output and give it to layer 2
    for (int i = 1; i <= n1; ++i) {
        for (int j = 1; j <= n2; ++j) {
            in2[j] += out1[i] * w1[i][j];
		}
    ...
    // apply second activation function on the output of the first layer
    for (int i = 1; i <= n2; ++i) {
		out2[i] = relu(in2[i]);
	}

    // factor in the weights from the second layer and give the output to the third layer
    for (int i = 1; i <= n2; ++i) {
        for (int j = 1; j <= n3; ++j) {
            in3[j] += out2[i] * w2[i][j];
		}
    ...
    // apply the activation function to the third (output) layer
    for (int i = 1; i <= n3; ++i) {
		out3[i] = relu(in3[i]);
	}
}
